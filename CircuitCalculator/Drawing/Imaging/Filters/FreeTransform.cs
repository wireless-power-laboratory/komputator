...
namespace CircuitCalculator.Imaging.Filters
{
    public class FreeTransform
    {
        PointF[] vertex = new PointF[4];
        CircuitCalculator.Geometry.Vector AB, BC, CD, DA;
        Rectangle rect = new Rectangle();
        CircuitCalculator.Imaging.ImageData srcCB = new ImageData();
        int srcW = 0;
        int srcH = 0;
        ...
        private void setVertex()
        {
            ...
            AB = new CircuitCalculator.Geometry.Vector(vertex[0], vertex[1]);
            BC = new CircuitCalculator.Geometry.Vector(vertex[1], vertex[2]);
            CD = new CircuitCalculator.Geometry.Vector(vertex[2], vertex[3]);
            DA = new CircuitCalculator.Geometry.Vector(vertex[3], vertex[0]);
        ...
        {
            if (!CircuitCalculator.Geometry.Vector.IsCCW(pt, vertex[0], vertex[1]))
            {
                if (!CircuitCalculator.Geometry.Vector.IsCCW(pt, vertex[1], vertex[2]))
                {
                    if (!CircuitCalculator.Geometry.Vector.IsCCW(pt, vertex[2], vertex[3]))
                    {
                        if (!CircuitCalculator.Geometry.Vector.IsCCW(pt, vertex[3], vertex[0]))
                            return true;
                    }
                }
            }
        ...
        private Bitmap getTransformedBitmap()
        {
            ...
            for (int y = 0; y < rect.Height; y++)
            {
                for (int x = 0; x < rect.Width; x++)
                {
                    ...
                    {
                        dab = Math.Abs((new CircuitCalculator.Geometry.Vector(vertex[0], srcPt)).CrossProduct(AB));
                        dbc = Math.Abs((new CircuitCalculator.Geometry.Vector(vertex[1], srcPt)).CrossProduct(BC));
                        dcd = Math.Abs((new CircuitCalculator.Geometry.Vector(vertex[2], srcPt)).CrossProduct(CD));
                        dda = Math.Abs((new CircuitCalculator.Geometry.Vector(vertex[3], srcPt)).CrossProduct(DA));
